...
function buildContent(input){
  if(input.action==='intake')return{value:`Turn these HVAC field notes into concise structured intake. Return valid JSON only with keys projectType, summary, extractedFields, missingQuestions. Keep summary under 80 words, include only fields supported by the notes, and ask at most 6 essential missing questions. Never guess equipment identifiers. Use "General HVAC Proposal" when the job is broad or does not match a specialized type. Use "Small Proposal" only when the user explicitly requests a short/small proposal or the work is clearly minor. Customer: ${clean(input.customer,200)}\nNotes: ${clean(input.description,6000)}`};
